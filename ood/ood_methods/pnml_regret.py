...
def get_pnml_regrets(load_x, test_ids, projection_matrix_path, predict_with_features_fn, 
    ...
    for uid in tqdm(test_ids):
        ...
        for projection_matrix_name in projection_matrices:
            projection_matrix = load(os.path.join(projection_matrix_path, 'experiment_0', projection_matrix_name)).astype(np.float64)
            results[projection_matrix_name][uid] = calculate_regret(probs, test_features, projection_matrix=projection_matrix)
